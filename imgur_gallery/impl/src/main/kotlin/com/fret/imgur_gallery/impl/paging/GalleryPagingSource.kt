package com.fret.imgur_gallery.impl.paging
...
abstract class GalleryPagingSource: PagingSource<Int, GalleryItemModel>() {
    abstract val section: Section
    abstract val sort: Sort
    abstract val window: Window

    interface Factory {
        fun create(section: Section = Section.hot, sort: Sort = Sort.viral, window: Window = Window.day): GalleryPagingSource
    }
}
...
class GalleryListPagingSourceImpl @AssistedInject constructor(
    ...
    @AssistedFactory
    @ContributesBinding(GalleryScope::class)
    interface Factory : GalleryPagingSource.Factory {
